...
public class Heap<V> {
    ...
    protected void swap(int h, int k) {
        ...
        Info switchedNode = new Info (c[h].value, c[h].priority);
        c[h].value = c[k].value;
        c[h].priority = c[k].priority;
        valPos.put(c[h].value, h);
        c[k].value = switchedNode.value;
        c[k].priority = switchedNode.priority;
    ...
    private void bubbleUp(int k) {
        ...
        assert 0 <= k  && k < size;
        int parent = (k-1)/2;
        while (c[parent].priority > c[k].priority) {
        	swap(k, parent);
        	k = parent;
        	parent = (k-1)/2;
        }
    ...
    private void bubbleDown(int k) {
    	...
    	//Loop runs until no longer can bubbleDown//
    	while (2*k + 1 < size) {
        	int smaller = smallerChildOf(k);
    		if (c[smaller].priority < c[k].priority){
    			swap(k, smaller);
    		} else {
    			return;
    		}
    ...
    protected int smallerChildOf(int n) {
        ...
        //Checks if a right child exists. If not, return left child//
        if (leftChildIndex + 1 == size)
        	return leftChildIndex;
        
        //Compare priorities of left and right child//
        if (c[leftChildIndex].priority < c[rightChildIndex].priority) 
        	return leftChildIndex;
    ...
    public void updatePriority(V v, double p) {
        ...
        if (!valPos.containsKey(v)) {
        	...
        	int ind = valPos.get(v);
        	double oldPriority = c[ind].priority;
        	c[ind].priority = p;
        	if (p > oldPriority)
        		bubbleDown(ind);
        	if (oldPriority > p)
